...
graph TB
    ...
    %% Workflow 3
    WF3 -->|Username lookup| GH
    GH -->|Repos + READMEs| WF3
    WF3 -->|Compiled repo data| GM
    GM -->|GitHub score + hidden gem| WF3
    WF3 -->|Combined score| WF4

    %% Workflow 4
    WF4 -->|Score 75-100| SL
    WF4 -->|Score 75-100: Shortlist| ML
    WF4 -->|Score 45-74: Assessment Link| ML
    WF4 -->|Score 0-44: Rejection + TF Link| ML
    TF -->|Webhook: Talent Pool Opt-in| WF4

    %% Workflow 5
    AS -->|Webhook: Assessment Submit| WF5
    WF5 -->|Answers + Context| GM
    GM -->|Assessment Score| WF5
    WF5 -->|Recalculated Score >= 75| SL
    WF5 -->|Final Decision Email| ML
    WF5 -->|Cron: Day 5 & 8 Nudges/Archive| ML

    %% Database Interactions (Dashed to keep main flow clean)
    WF1 -.->|Read/Write| DB1
    WF1 -.->|Write| DB2
    WF2 -.->|Read| DB1
    WF2 -.->|Write| DB2
